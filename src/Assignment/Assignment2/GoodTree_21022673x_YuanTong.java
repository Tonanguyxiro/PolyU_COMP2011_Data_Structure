...
import Lecture.lec3_sample.Stack;
import Lecture.lec4_sample.LinkedList;
...
public class GoodTree_21022673x_YuanTong {
    ...
    private int recSize(Node e) {
        if(e==null){
            ...
            return 1+recSize(e.lc)+recSize(e.rc);
        }
    }
    ...
    public int recMin() { return recMin(root); }
    ...
    private int recMin(Node e){
        if(e.lc==null&&e.rc==null){
            ...
            return Math.min(recMin(e.rc), recMin(e.lc));
        }
    }
    ...
    private int leaves(Node e) {
        ...
        while (myStack.isEmpty() == false) {
            ...
            // Work
            if (current.lc==null&&current.rc==null) leaves++;
    ...
    private int fullNodes(Node e) {
        ...
        while (myStack.isEmpty() == false) {
            ...
            // Work
            if (current.lc!=null&&current.rc!=null) fullNodes = current.element;
...
//        else root = merge(root, t2.root);
    ...
    // This method is used for test, delete it when hand in
    private Node randomMerge(Node root_1, Node root_2) {
        if (root_1.element > root_2.element) {
            if (Math.random()>0.5){
                if (root_1.rc != null) {
                    Node subtree = root_1.rc;
                    root_1.rc = randomMerge(subtree, root_2);
                }
                else {
                    root_1.rc = root_2;
                }
                ...
                if (root_1.lc != null) {
                    Node subtree = root_1.lc;
                    root_1.lc = randomMerge(subtree, root_2);
                }
                else {
                    root_1.lc = root_2;
                }
            ...
            if (Math.random()>0.5) {
                if (root_2.rc != null) {
                    Node subtree = root_2.rc;
                    root_2.rc = randomMerge(subtree, root_1);
                }
                else {
                    root_2.rc = root_1;
                }
                ...
                if (root_2.lc != null) {
                    Node subtree = root_2.lc;
                    root_2.lc = randomMerge(subtree, root_1);
                }
                else {
                    root_2.lc = root_1;
                }
    ...
    private String toString(Node root)
    {
        ...
        return builder.append(root.element+" ").toString();
    }
    ...
    private void printTree(){
        ...
        if (root!=null) node_this_layer[0] = root;
        else System.out.println("Error");
    ...
    private void construct_tree(int[][] a){
        Node[] node_this_layer = new Node[1];
        for (int layer = 0; layer < a.length-1; layer++) {
            if (root!=null) node_this_layer[0] = root;
            else {
                root = new Node(1);
                node_this_layer[0] = root;
            }
            ...
            for (int i = 0; i < node_this_layer.length; i++) {
                if (a[layer+1][i*2] == -1) {
                    node_next_layer[i*2] = null;
                    node_this_layer[i].lc = null;
                }
                else {
                    node_this_layer[i].lc = new Node(a[layer+1][i*2]);
                    node_next_layer[i*2] = node_this_layer[i].lc;
                }

                if (a[layer+1][i*2+1] == -1) {
                    node_next_layer[i*2+1] = null;
                    node_this_layer[i].rc = null;
                }
                else {
                    node_this_layer[i].rc = new Node(a[layer+1][i*2+1]);
                    node_next_layer[i*2+1] = node_this_layer[i].rc;
                }
            }
            node_this_layer = node_next_layer;
            this.printTree();
...
//        t1.merge(t2);
        ...
        GoodTree_21022673x_YuanTong t1 = new GoodTree_21022673x_YuanTong(1);
        t1.Full_tree();
        t1.printTree();
...
//
        ...
        System.out.println("By recursion: " + t1.recSize());

        System.out.println("Min: ");
        System.out.println("By iteration: " + t1.min());
        System.out.println("By recursion: " + t1.recMin());

        System.out.println("Leaves: ");
        System.out.println("By iteration: " + t1.leaves());
        System.out.println("By recursion: " + t1.recLeaves());

        System.out.println("Full nodes");
        System.out.println("By iteration: " + t1.fullNodes());
